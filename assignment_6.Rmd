...
While the model gives us a better fit when we interact the completion of vulnerability assessment with the percentage of energy use from renewable sources, the relationship is not strong. 
```{r}
...
Curious about whether the commencement of vulnerability assessment, which is a binary variable, instead of completion of vulnerability assessment which has three levels, would change the model fit, we conducted the following to examine. It appears that the interaction does not improve the model fit, giving us a R value of 0.45 compared to the earlier 0.48. 

```{r}
coeff_names_interaction_2 <- c("Constant" = "(Intercept)",
                 "Non-C40 City" = "C40_StatusNo",
                 "Conducted vulnerability assessment " = "VacommencedYes",
                 "Population Density\n(per sq km) Centered" = "pop_dens_centered",
                 "Percentage of Energy Use\nfrom Renewable Sources % Centered" = "PercentRenew_centered",
                 "Total Emissions\n(metric tonnes CO2e) logged" = "log_Emission")

centered_model_log2 <- lm(WellBeing ~ C40_Status + Vacommenced + PercentRenew_centered + pop_dens_centered + log_Emission, data_log)

interaction_model_3 <- lm(WellBeing ~ C40_Status + Vacommenced + PercentRenew_centered + pop_dens_centered + log_Emission + Vacommenced*PercentRenew_centered, data_log)

interaction_model_4 <- lm(WellBeing ~ C40_Status + Vacommenced + PercentRenew_centered + pop_dens_centered + log_Emission + C40_Status*PercentRenew_centered, data_log)

export_summs(centered_model_log2, interaction_model_3, interaction_model_4,
             error_format = "(p = {p.value})",
             error_pos = "same",
             model.names = c("logged",
